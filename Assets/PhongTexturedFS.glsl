...
vec3 phongModel(vec3 fragPosition, vec3 fragNormal, vec3 colour) {
...
void main() {
    vec3 colour = texture(Kd, vertexUV).xyz;

    vec3 phongColour = phongModel(vertexPosition, vertexNormal, colour) * lightColour;
    ...
    vec3 ambientValue = ambientIntensity * ambientCoefficient * colour;
