...
// types instead of, for example, just allocating RenderObjects directly
...
// important) reason is that we will want RenderObjects to stick around
// even after lua is done with them, after all, we want to pass them
// back to the program. If we let Lua allocate RenderObjects directly,
...
// Allocated by Lua to represent lists
struct gr_list_ud {
    RenderList* list;
};

// Allocated by Lua to represent objects
struct gr_object_ud {
    RenderObject* object;
};
...
extern "C"
int gr_list_cmd(lua_State* L) {
    GRLUA_DEBUG_CALL;

    gr_list_ud* data = (gr_list_ud*)lua_newuserdata(L, sizeof(gr_list_ud));
    data->list = 0;

    std::cout << "Loading list" << std::endl;
    data->list = new RenderList();

    luaL_getmetatable(L, "gr.list");
    lua_setmetatable(L, -2);
...
extern "C"
int gr_object_cmd(lua_State* L) {
    GRLUA_DEBUG_CALL;

    gr_object_ud* data = (gr_object_ud*)lua_newuserdata(L, sizeof(gr_object_ud));
    data->object = 0;
...
extern "C"
int gr_list_gc_cmd(lua_State* L) {
    GRLUA_DEBUG_CALL;

    gr_list_ud* data = (gr_list_ud*)luaL_checkudata(L, 1, "gr.list");
    // Note that we don't delete the list here. This is because we still
    // want the scene to be around when we close the lua interpreter,
    // but at that point everything will be garbage collected.
    //
    // If data->list happened to be a reference-counting pointer, this
    // will in fact just decrease lua's reference to it, so it's not a
    // bad thing to include this line.
    data->list = 0;
...
extern "C"
int gr_object_gc_cmd(lua_State* L) {
    GRLUA_DEBUG_CALL;
    gr_object_ud* data = (gr_object_ud*)luaL_checkudata(L, 1, "gr.object");
    data->object = 0;
    return 0;
}
...
static const luaL_Reg grlib_functions[] = {
        {"make_list", gr_list_cmd},
        {"make_object", gr_object_cmd},
        {0, 0}
};

// This is where all the member functions for "gr.object" objects are
// declared.
//
// If you want to add a new member function for gr.object, add it
// here.
static const luaL_Reg grlib_object_methods[] = {
        {"__gc", gr_object_gc_cmd},
...
// These methods are all member functions of "gr.list"
static const luaL_Reg grlib_list_methods[] = {
        {"__gc", gr_list_gc_cmd},
        {"add_object", gr_list_add_object_cmd},
        {0, 0}
};
...
RenderList* run_lua(const std::string& filename)
{
    ...
    luaL_newmetatable(L, "gr.list");
    lua_pushstring(L, "__index");
    lua_pushvalue(L, -2);
    lua_settable(L, -3);

    // Load the gr.list methods
    luaL_setfuncs(L, grlib_list_methods, 0);

    luaL_newmetatable(L, "gr.object");
    lua_pushstring(L, "__index");
    lua_pushvalue(L, -2);
    lua_settable(L, -3);

    // Load the gr.object methods
    luaL_setfuncs(L, grlib_object_methods, 0);
